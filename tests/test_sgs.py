...
class TestSGSFactory:
    ...
    def test_get_model_returns_sgs(self, model_id: int) -> None:
        """Test that factory returns valid SGS model."""
        input_obj = MockInput()
        model = SGS.get_model(model_id, input_obj)

        assert model is not None
        assert hasattr(model, "compute")

    def test_get_model_invalid_id(self) -> None:
        """Test that invalid model ID raises error."""
        input_obj = MockInput()

        with pytest.raises(ValueError):
            SGS.get_model(99, input_obj)
...
class TestSGSModels:
    ...
    def test_sgs_returns_tau(self, model_id: int, test_field: tuple) -> None:
    ...
    @pytest.mark.parametrize("model_id", [1, 2, 3])
    def test_sgs_returns_coeff(self, model_id: int, test_field: tuple) -> None:
    ...
    @pytest.mark.parametrize("model_id", [1, 2, 3])
    def test_sgs_tau_finite(self, model_id: int, test_field: tuple) -> None:
    ...
    def test_deardorff_returns_tke_sgs(self, test_field: tuple) -> None:
        """Test that Deardorff model returns subgrid TKE."""
        u, dudx = test_field
        input_obj = MockInput()
        model = SGS.get_model(4, input_obj)
    ...
    def test_deardorff_tke_positive(self, test_field: tuple) -> None:
...
class TestSGSPhysics:
    ...
    def test_smagorinsky_dissipative(self) -> None:
        ...
        model = SGS.get_model(1, input_obj)
    ...
    def test_dynamic_model_adapts_coefficient(self) -> None:
        ...
        input_obj = MockInput(nxLES=nx)
        model = SGS.get_model(2, input_obj)
